...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        # model = models.mobilenet_v2(pretrained=False, progress=True, num_classes=6)
        model = MobileNetV2Cus(6)
        ...
        for dir_name in dir_names:
            ...
            for section_name in section_names:
                ...
                for file_idx, file_path in tqdm(enumerate(files), total=len(files)):
                    ...
                    data = torch.from_numpy(data.reshape(data.shape[0], 1, param["feature"]["n_frames"], param["feature"]["n_mels"]))
                    data = torch.cat((data, data, data), 1).to(device=device, dtype=torch.float)
                    # emb = model.features(data)
                    # emb = torch.nn.functional.adaptive_avg_pool2d(emb, (1, 1))
                    # emb = torch.squeeze(emb).detach().cpu().numpy()
                    emb = model.emb(data).detach().cpu().numpy()
                    if section_idx == 0:
                        y_pred[file_idx] = np.mean(ml0.decision_function(emb))
                    elif section_idx == 1:
                        y_pred[file_idx] = np.mean(ml1.decision_function(emb))
                    else:
                        y_pred[file_idx] = np.mean(ml2.decision_function(emb))
